...
class hlpay_plugin
{
	...
	static private function addOrder($way_code, $sub_appid=null, $sub_openid=null){
		...
		if($sub_appid && $sub_openid){
			...
			$param += [
				'channel_extra' => json_encode(['user_id' => $sub_openid]),
			];
		}
	...
	static public function alipayjs(){
		...
		try{
			$pay_info = self::addOrder($channel['appswitch'] == 1 ? 'onAlipayPub' : 'offAlipayPub', null, $user_id);
		}catch(Exception $ex){
			return ['type'=>'error','msg'=>'支付宝支付下单失败！'.$ex->getMessage()];
		}
		...
		return ['type'=>'page','page'=>'alipay_jspay','data'=>['alipay_trade_no'=>$pay_info, 'redirect_url'=>$redirect_url]];
	}
